...
//Copiado de stackoverflow, testeado en regex101
function numeroConPunto(x) {
...
function agregarItemsLiEnPadreUl(padre, lista){
...
//Agrega un elemento li en el elemento padre del dom, el cual para mantener coherencia y semántica HTML debe ser ol o ul
function agregarItemLiEnPadreOloUl(padre, texto){
...
ready(function(){
  ...
  function actualizarTotal(){
    ...
    spanMostrarTotal.appendChild(document.createTextNode(numeroConPunto(total)));
  }

  //Agrega un "listener" a cada input checkbox
  for( inputCheckboxIngrediente of inputsCheckboxIngredientes){
    inputCheckboxIngrediente.addEventListener("click", actualizarListaDeIngredientes);
  }
  ...
  function actualizarListaDeIngredientes(event){ // event.target == this ???
    ...
    if (ingredienteEsSeleccionado){ //ingrediente es agregado a la lista
      ...
      const possibleIndex = ingredientesSeleccionados.indexOf(this.value)
      const index = possibleIndex != -1 ? possibleIndex : false;
    ...
    inputMostrarExtra.setAttribute('value', numeroConPunto(precioIngredientesExtras));
    actualizarTotal();
  }
  ...
  botonEnviar.addEventListener("click", (event) => {
    ...
    else alert(`Su propina de $${numeroConPunto(propina)} ha sido enviada.`);
  })
});
